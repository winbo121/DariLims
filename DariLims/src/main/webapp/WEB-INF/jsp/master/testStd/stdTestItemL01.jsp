...
// 		ajaxCombo("unit", "C08", "ALL", "");
// 		ajaxCombo("result_type", "C31", "ALL", "");		
		
		stdTestItemGrid('master/selectStdTestItemList.lims', 'stdTestItemForm', 'stdTestItemGrid');	
		$('#stdTestItemGrid').clearGridData(); // 최초 조회시 데이터 안나옴
	});
...
// 				fnCheckAction(grid);
	...
	function stdTestItemGrid(url, form, grid) {
// 		var lastRowId;
		$('#' + grid).jqGrid({
			datatype : function(json) {
				fnGridData(url, form, grid);
	...
	function btn_Insert_Sub2_onclick(){	
		...
		data += '&test_std_no=' + $("#stdTestItemForm").find("#test_std_no").val() + '&rev_no=' + $("#stdTestItemForm").find("#rev_no").val();
//  		alert(data);	
		var json=fnAjaxAction('master/insertStdTestItem.lims', data);
		if (json == null) {
			$.showAlert("저장 실패하였습니다.");
		} else {
			$('#stdTestItemGrid').trigger('reloadGrid');
