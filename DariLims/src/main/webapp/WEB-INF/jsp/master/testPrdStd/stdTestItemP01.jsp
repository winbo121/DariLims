...
<script type="text/javascript">
	$(function() {
		grid('master/selectStdTestPrdItemList.lims?pageType=all', 'form', 'grid');
		grid('', 'stdTestPrdItemForm', 'stdTestPrdItemGrid');
		...
		$(window).bind('resize', function() {
			$("#stdTestPrdItemGrid").setGridWidth($('#view_grid_sub').width(), false);
		}).trigger('resize');
		...
		$('#' + grid).jqGrid({
			...
			onSelectRow : function(rowId, status, e) {
				if (grid == 'grid') {
					...
					//부모창에서 받아온 항목들 체크
					var check = stdTestPrdItems(rowId);
					if (check) {
			...
			onSelectAll : function(selar, status) {
				if (!allselect) {
					...
					for ( var row in selar) {
						...
						var sel_ids = $('#stdTestPrdItemGrid').jqGrid("getDataIDs");
	...
	//부모창에서 받아온 선택된 시험항목들
	function stdTestPrdItems(rowId) {
		var check = false;
		var pre_ids = window.dialogArguments["ids"];
	...
	function btn_Insert_onclick() {
		...
		var selar = $("#stdTestPrdItemGrid").jqGrid('getDataIDs');
		if (selar.length > 0) {
			var i = 0;
			for ( var row in selar) {
				var r = $('#stdTestPrdItemGrid').jqGrid('getRowData', selar[row]);
				returnValue[i] = r;
				i++;
			}
