...
<script type="text/javascript">
	...
	function btn_Insert_onclick() {
		...
		if (json == null) {
			...
			var addData = {
				"mtlr_no" : json,
				"h_mtlr_info" : $('#buyingConfirmationModifyForm').find("#h_mtlr_info option:selected").text(),
				"m_mtlr_info" : $('#buyingConfirmationModifyForm').find("#m_mtlr_info option:selected").text(),
				// 			"h_mtlr_info": $('#buyingConfirmationModifyForm').find("#h_mtlr_info").val(),
				// 			"m_mtlr_info": $('#buyingConfirmationModifyForm').find("#m_mtlr_info").val(),
				"item_nm" : $('#buyingConfirmationModifyForm').find('#item_nm').val(),
				// 			"unit" : $('#buyingConfirmationModifyForm').find('#unit').val(),
				"unit" : $('#buyingConfirmationModifyForm').find("#unit option:selected").text(),
				"use" : $('#buyingConfirmationModifyForm :radio[id="use"]:checked').val(),
				// 			"use" : $('#buyingConfirmationModifyForm').find('#use').val(),
				"spec1" : $('#buyingConfirmationModifyForm').find('#spec1').val(),
				"spec2" : $('#buyingConfirmationModifyForm').find('#spec2').val(),
				"spec_etc" : $('#buyingConfirmationModifyForm').find('#spec_etc').val(),
				"content" : $('#buyingConfirmationModifyForm').find('#content').val(),
				"etc" : $('#buyingConfirmationModifyForm').find('#etc').val()
			};
		...
		//$.showAlert('저장 되었습니다.');
		opener.btn_Search_sub_onclick2();
	}
			...
			<form id="buyingConfirmationModifyForm" name="buyingConfirmationModifyForm" onsubmit="return false;">
				<div class="sub_purple_01">
					<table  class="select_table" >
						<tr>
							<td width="60%" class="table_title"><span>■</span> 시약/실험기기 수정</td>
							...
							</span> <span class="button white mlargeg auth_save" id="btn_Insert" onclick="btn_Insert_onclick();">
									<button type="button">저장</button>
							</span> <span class="button white mlargep" id="btn_New_Close" onclick="btn_New_Close_onclick();">
					...
					<table  class="list_table" >
						<tr>
							<th>대분류</th>
							<td><select name="h_mtlr_info" id="h_mtlr_info_detail" onchange="comboReload_detail();" class="w100px">
							...
							<th>중분류</th>
							<td><select name="m_mtlr_info" id="m_mtlr_info" class="w120px"></select></td>
						</tr>
						<tr>
							<th>시약/실험기구명</th>
							<!--한글우선입력-->
							<td>
								...
								<input name="item_nm" id="item_nm" type="text" class="inputhan" value="${detail.item_nm}" /></td>
							<th>단위</th>
							<td><select name="unit" id="unit" class="w120px"></select></td>
						</tr>
						<tr>
							<th>제조사</th>
							<td><input name="spec1" id="spec1" type="text" value="${detail.spec1}" /></td>
							<th>Cas no.</th>
							<td><input name="spec2" id="spec2" type="text" value="${detail.spec2}" /></td>
						</tr>
						<tr>
							<th>Cat # (제품번호)</th>
							<td><input name="spec_etc" id="spec_etc" type="text" value="${detail.spec_etc}" /></td>
							<th>Lot # (로트번호)</th>
							<td><input name="use" id="use" type="text" value="${detail.use}" /></td>
						</tr>
						<tr>
							<th>사용여부</th>
							<td colspan="3"><input type="radio" name="use_flag" id="use_flag" style="width: 15px; margin-left: 15px; top: 1px;" value="Y" <c:if test="${detail.use_flag == 'Y' || detail.use_flag == '' || detail.use_flag == null}">checked="checked"</c:if> /> 사용함 <input type="radio" name="use_flag" id="use_flag" style="width: 15px; margin-left: 15px; top: 1px;" value="N" <c:if test="${detail.use_flag == 'N'}">checked="checked"</c:if> /> 사용안함</td>
						</tr>
						<tr>
							<th>내용</th>
							<td colspan="5"><textarea rows="3" cols="90%" style="border: 1px solid #afafaf;" name="content" id="content" class="inputhan">${detail.content}</textarea></td>
						</tr>
						<tr>
							<th>비고</th>
							<td colspan="3"><textarea rows="3" cols="90%" style="border: 1px solid #afafaf;" name="etc" id="etc" class="inputhan">${detail.etc}</textarea></td>
						</tr>
					</table>
				</div>
			</form>
